...
find_opt_hidden_units <- function(minimize_false_pos = F){
  ...
  for(num_units in 1:10){
    net = nnet(type ~ ., data = spam_train, size = num_units, trace = F, rang = 0.5, maxit = 100) #weights are randomly initialized by default
...
find_opt_decay_rate <- function(opt_hidden_units, minimize_false_pos = F){
  ...
  for(decay in seq(0,1, by = 0.1)){
    errors_tmp <- c()
    for(i in 1:10){
       net = nnet(type ~ ., data = spam_train, size = opt_hidden_units, decay = decay, trace = F, rang = 0.5, maxit = 100)
       class_probs <- predict(net, select(spam_test, -type), type = "raw")
