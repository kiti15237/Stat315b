...
find_opt_hidden_units <- function(minimize_false_pos = F){
  ...
  for(num_units in 1:10){
    net = nnet(type ~ ., data = spam_train, size = c(num_units), trace = F, rang = 0.5) #weights are randomly initialized by default
  ...
  if(minimize_false_pos == F){
    for(name in names(nets)){
      class_preds <- predict(nets[[name]], select(spam_test, -type))
      error = sum(round(class_preds) != spam_test$type) / length(class_preds)
      
      class_preds_list[[`name`]] <- class_preds
      errors[[`name`]] <- error
      print(paste("Training model with ", name, " units", ", error is ", error))
    }
    
  }
  if(minimize_false_pos == T){
    for(name in names(nets)){
      tmp = limit_false_pos(false_pos_limit = 0.01, net = nets[[name]], data = spam_test)
        class_preds <- tmp[[1]]
        error <- tmp[[2]]
        false_pos_rate <- tmp[[3]]
      print(paste("Training model with ", name, " units", ", error is ", error, " false pos rate is ", false_pos_rate))
      class_preds_list[[`name`]] <- class_preds
      errors[[`name`]] <- error
    }
  }
  ...
  return(opt_hidden_units)

}

limit_false_pos <- function(false_pos_limit = .01, net, data){
  ...
  return(list(class_preds, error, false_pos_rate))
}

```
which structural model performs best at classifying on the test set?
```{r}
opt_hidden_units <- find_opt_hidden_units(minimize_false_pos = F)
...
find_opt_decay_rate <- function(opt_hidden_units, minimize_false_pos = F){
  ...
  for(decay in seq(0,1, by = 0.1)){
    errors_tmp <- c()
    for(i in 1:10){
       net = nnet(type ~ ., data = spam_train, size = c(opt_hidden_units), decay = decay, trace = F, rang = 0.5)
        
       if(minimize_false_pos == T){
          tmp = limit_false_pos(false_pos_limit = 0.01, net = net, data = spam_test)
          class_preds <- tmp[[1]]
          error <- tmp[[2]]
          false_pos_rate <- tmp[[3]]
       }
       if(minimize_false_pos == F){
          class_preds <- predict(net, select(spam_test, -type))
          error = sum(round(class_preds) != spam_test$type) / length(class_preds)
       }
...
The final best model has `r opt_hidden_units`  hidden units, a decay rate of `r opt_decay` and an error of `r min_error` on the test set


c) The goal is now to obtain a spam filter. Repreat the previous point requiring this time the proportion of misclassified good emails to be less than 1%.
```{r false_pos_limited}
...
print(paste("After mandating the false positive rate be less than 1 %, optimal number of hidden units is: ", opt_hidden_units,
      " optimal decay rate is ", opt_decay, "and the test error obtained by this model is ", min_error))
```
