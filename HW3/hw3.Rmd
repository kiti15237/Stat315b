...
We wish to find all symmetric matrices $\Sigma$ such that there exists some orthogonal basis $\{v_1....v_p\}$, so that $B(x + v | \mu_m, \Sigma) = $B(x|\mu_m, \Sigma)$ for all $v_j$ except one.  This is what it means to vary only in one direction.  Notice that $(x+v-\mu)^T\Sigma(x+v-\mu)$ (and, by extension  $B(x + v | \mu_m, \Sigma)$, is invariant to $v$ whenever $v$ is in the nullity of $\Sigma$.  For $\Sigma$ to vary in only one direction, then, it must have nullity $p-1$ and rank $1$.  Thus, $B(x|\mu_m, \Sigma)$ varies in only one direction whenever $\Sigma$ is in the set of symmetric, rank 1 matrices. This makes sense, because then $B(x|\mu_m, \Sigma)$ is proportional to some univariate normal p.d.f..
