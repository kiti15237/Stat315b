...
(misclassification.error <- (1 - sum(spam_test$type == gbm0.labels) / nrow(spam_test))*100)
test.pos.inds <- which(spam_test$type == 1)
test.neg.inds <- which(spam_test$type == 0)
(false.negs <- (1 - sum(spam_test[test.pos.inds,]$type == gbm0.labels[test.pos.inds]) / length(test.pos.inds))*100)
(false.pos <- (1 - sum(spam_test[test.neg.inds,]$type == gbm0.labels[test.neg.inds]) / length(test.neg.inds))*100)
