...
#####################################################
1) Differentiate and find the form of the solution by setting it equal to 0
...
We now consider the elastic net:
$$ (y-xa)^2 + \sum_{j=1}^n (\gamma - 1)a_j^2/2 + (2-\gamma)|a_j| $$
We would like to take the derivative w.r.t $a_j$ as we did above, however, we cannot differentiate the absolute value. We therefore take directional derivatives. We take $e_j$ to be the cardinal direction of j.

$$D_{e_j} = lim_{\epsilon -> 0} \frac{1}{\epsilon} [ R(a + \epsilon e_j) - R(a) + \lambda P(a + \epsilon e_{j}) - \lambda P(a)] $$
$$R(a + \epsilon e_j) - R(a) = x_j(y - x(a + \epsilon e_j)) $$
$$\lambda P(a + \epsilon e_1) - \lambda P(a) = (\gamma - 1) (a_j + \epsilon e_j)^2 - (\gamma - 1)(a_j)^2 + (2 - \gamma)|\epsilon e_j| = (\gamma -1 )(2a_j \epsilon e_j + \epsilon ^2 e_j^2) + (2 - \gamma)|\epsilon e_j|$$

#eventually this should lead us to the conclusion that $x_j(y-xa)$ can be in some range when $a_j = 0$ and is a minimum. This can happen with an actual probability, as opposed to the other scenario where that value had to be equal to 0


# Problem 4:
INCOMPLETE: I"M NOT SURE WHETHER I'M ON THE RIGHT TRACK
...
gbm0 <- gbm(type ~ ., data=spam_train, train.fraction=0.8, interaction.depth=4, shrinkage = 0.05, n.trees=2500, bag.fraction=0.5, cv.folds=5, distribution="bernoulli", verbose=F)
gbm0.predict <- predict(gbm0, spam_train, type="response", n.trees=300)
gbm.perf(gbm0, method="cv")
summary(gbm0, main="RELATIVE INFLUENCE OF ALL PREDICTORS")

gbm0.test <- predict(gbm0, spam_test, type="link", n.trees=300)
gbm0.labels <- as.numeric(gbm0.test >= 0)
...
Misclassification: `r misclassification.error`
Spam emails misclassified: `r false.negs`
Good emails misclassified: `r false.pos`
...
# Problem 7:
**(15)Regression: California Housing.The data setcalif_stats315B.csv consists of aggregated data from 20,640 California census blocks (from the 1990 census). The goal is to predictthe median house value in each neighborhood from the others described in calif_stats315B.txt.Fit a gbm model to the data and write a short report that shouldincludeat least**
...
gbm.perf(gbm.occup, method="cv")

gbm.predict <- predict(gbm.occup, occup[test_ind,-1], type = "response")
